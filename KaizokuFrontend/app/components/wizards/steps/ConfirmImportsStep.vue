...
function scheduleSyncToBackend(importInfo: ImportInfo) {
  ...
  debounceTimers[key] = setTimeout(async () => {
    try {
      await setupWizardService.updateImport(importInfo)
    } catch {
...
onUnmounted(() => {
  for (const key of Object.keys(debounceTimers)) {
    ...
    if (imp) setupWizardService.updateImport(imp).catch(() => {})
    delete debounceTimers[key]
  }
})
...
// Sort series: preferred first, then by importance (lower = higher priority)
function sortedSeries(series: SmallSeries[]): { series: SmallSeries; origIdx: number }[] {
  return series
...
function movePriority(importPath: string, seriesIndex: number, direction: 'up' | 'down') {
  ...
  // Find the adjacent series to swap with (sorted by importance)
  const sorted = series.map((s, i) => ({ s, i })).sort((a, b) => a.s.importance - b.s.importance)
  const sortedIdx = sorted.findIndex(item => item.i === seriesIndex)
  const swapSortedIdx = direction === 'up' ? sortedIdx - 1 : sortedIdx + 1
  if (swapSortedIdx < 0 || swapSortedIdx >= sorted.length) return

  const swapOrigIdx = sorted[swapSortedIdx].i
      ...
      <UCard v-for="item in filteredImports" :key="item.path" class="w-full">
        ...
        <div class="flex gap-3">
          ...
          <div class="flex-1 min-w-0 space-y-2">
            ...
            <!-- Series Grid — sorted: preferred first, then by priority -->
            <div v-if="item.series?.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
              <div
                v-for="{ series, origIdx } in sortedSeries(item.series)"
                :key="`${series.id}-${series.provider}-${series.scanlator ?? ''}`"
              ...
              >
                ...
                <!-- Controls — priority arrows + switches -->
                <div
                ...
                >
                  ...
                  <div class="inline-flex items-center gap-0.5">
                    ...
                    <button
                      class="p-0.5 rounded hover:bg-black/10 dark:hover:bg-white/20 disabled:opacity-30 disabled:cursor-not-allowed"
                      :disabled="series.importance >= (item.series?.length ?? 1) - 1"
