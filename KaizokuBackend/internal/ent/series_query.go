...
type SeriesQuery struct {
	...
	withLatestSeries *LatestSeriesQuery
...
// QueryLatestSeries chains the current query on the "latest_series" edge.
func (_q *SeriesQuery) QueryLatestSeries() *LatestSeriesQuery {
	query := (&LatestSeriesClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *SeriesQuery) Clone() *SeriesQuery {
	...
	return &SeriesQuery{
		...
		withLatestSeries: _q.withLatestSeries.Clone(),
...
// WithLatestSeries tells the query-builder to eager-load the nodes that are connected to
// the "latest_series" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *SeriesQuery) WithLatestSeries(opts ...func(*LatestSeriesQuery)) *SeriesQuery {
	query := (&LatestSeriesClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withLatestSeries = query
	return _q
}
...
func (_q *SeriesQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Series, error) {
	...
	if query := _q.withLatestSeries; query != nil {
		if err := _q.loadLatestSeries(ctx, query, nodes,
			func(n *Series) { n.Edges.LatestSeries = []*LatestSeries{} },
			func(n *Series, e *LatestSeries) { n.Edges.LatestSeries = append(n.Edges.LatestSeries, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SeriesQuery) loadLatestSeries(ctx context.Context, query *LatestSeriesQuery, nodes []*Series, init func(*Series), assign func(*Series, *LatestSeries)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Series)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(latestseries.FieldSeriesID)
	}
	query.Where(predicate.LatestSeries(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(series.LatestSeriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.SeriesID
		if fk == nil {
			return fmt.Errorf(`foreign-key "series_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "series_id" returned %v for node %v`, *fk, n.ID)
